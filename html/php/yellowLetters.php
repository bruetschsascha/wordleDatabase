...
for($i = 0; $i < count($yellowLettersArray2D); $i++) {
    for($j = 0; $j < count($yellowLettersArray2D[$i]); $j++) {
        for($k = 0; $k < count($grayLettersArray); $k++){
            if($yellowLettersArray2D[$i][$j] != " " && $yellowLettersArray2D[$i][$j] != null) {
                if($yellowLettersArray2D[$i][$j] != $grayLettersArray[$k]){
                    $yellowLettersQuery = $yellowLettersQuery .  'AND word LIKE "%' . $yellowLettersArray2D[$i][$j] . '%" ';
                }
                elseif($yellowLettersArray2D[$i][$j] == $grayLettersArray[$k]) {
                    $yellowLettersQuery = $yellowLettersQuery .  'AND word LIKE "%' . $yellowLettersArray2D[$i][$j] . '%'. $yellowLettersArray2D[$i][$j] . '%"';
                }
